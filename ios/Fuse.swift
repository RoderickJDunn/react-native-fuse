...
public class Fuse {
    ...
    public typealias SimpleSearchResult = (index: Int, score: Double)
    ...
    public func search(_ pattern: Pattern?, in aString: String) -> (score: Double, ranges: [CountableClosedRange<Int>])? {
        ...
        if (abs(textLength - pattern.text.count) > self.lengthWobble) {
            // DEV: Test return early if lengths differ at all
            return nil
        }
    ...
    public func searchQuick(_ pattern: Pattern?, in aString: String) -> Double? {
        guard let pattern = pattern else {
            return nil
        }
...
extension Fuse {
    ...
    public func search(_ text: String, in aList: [String]) -> [Int] {
        print("searching list")
        let pattern = self.createPattern(from: text)
        
//        var items = [SearchResult]()
        var items: [SimpleSearchResult] = []
        
        for (index, item) in aList.enumerated() {
            if let score = self.searchQuick(pattern, in: item) {
//                items.append((index, result.score, result.ranges))
                items.append((index, score))
            }
        }
